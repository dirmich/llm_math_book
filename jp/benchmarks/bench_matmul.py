...
# === Bench 1: 行列積 テキスト CPU vs GPU ===
def main():
    print("=== Bench 1: 行列積 テキスト CPU vs GPU ===\n")
    ...
    ax.set_xlabel('行列 テキスト n'); ax.set_ylabel('時間 (ms)')
    ax.set_title('行列積 ベンチマーク: $O(n^3)$'); ax.legend(); ax.grid(True, alpha=0.3, which='both')
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / 'bench_matmul.png', dpi=100, bbox_inches='tight')
    plt.show()
    print(f"\nグラフ テキスト: {RESULTS_DIR / 'bench_matmul.png'}")
