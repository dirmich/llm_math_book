...
# === Bench: Attention シーケンス長テキスト ===

import torch.nn.functional as F

def main():
    print("=== Attention ベンチマーク ===")
    ...
    ax.set_xlabel('シーケンス長 n'); ax.set_ylabel('時間 (ms)')
    ax.set_title('Attention $O(n^2 d)$ ベンチマーク'); ax.legend(); ax.grid(True, alpha=0.3, which='both')
    plt.tight_layout()
